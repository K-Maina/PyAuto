...
TheFHunter.py is a script that recursively searches a file or files by format, word or by name in a directory or directories.
If TheFHunter.py finds a files, it will perform an action given to it such as deleting files, encrypting files, zipping files,
counting files, moving and copying files to other directory.
As for counting files, the result can be saved in csv format.

DISCLAIMER
__________

This script is meant to be used for research and educational purpose.
...
banner = """
    ...
    Let's hunt them down...
        Don't worry guys, i mean the files.
                   ((((((-:-))))))
    _______________________________________
    version 1.0
"""
...
def move_file(files):
    ...
    move_path = input("[+] Enter the destination directory path : ")
    try:
...
def zip_file(files):
    """Zips a files."""
    zip_path = os.path.join(os.getcwd(), 'result.zip')
    try:
...
def count_file(files):
    ...
    if option == 'yes':
        write_csv(data, os.path.join(os.getcwd(), 'result.csv'))


def perform_action(files, action_type):
    """Performs actions."""
    if not action_type:
        return files
...
def main():
    ...
    parser.add_argument('-e', '--formats', nargs = '+', dest = 'format', metavar = 'Format', type = str, help = 'Searches file or files by their formats.')
    ...
    parser.add_argument('-a', '--action', choices = ['delete', 'encrypt', 'move', 'copy', 'zip', 'count'], dest = 'action', help = 'Action to be performed when files are found.')
    ...
    if target_files:
        ...
        print()
        perform_action(target_files, args.action)
    else:
        termcolor.cprint("[-] File not found.", 'red')
    print('Done.')
